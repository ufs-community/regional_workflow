...
#-----------------------------------------------------------------------
#
# Check that CDATE_FIRST_CYCL and CDATE_LAST_CYCL are strings consisting 
# of exactly 10 digits.
#
#-----------------------------------------------------------------------
#
CDATE_OR_NULL=$( printf "%s" "$CDATE_FIRST_CYCL" | sed -n -r -e "s/^([0-9]{10})$/\1/p" )
if [ -z "${CDATE_OR_NULL}" ]; then
  print_err_msg_exit "\
CDATE_FIRST_CYCL must be a string consisting of exactly 10 digits of the 
...
CDATE_OR_NULL=$( printf "%s" "$CDATE_LAST_CYCL" | sed -n -r -e "s/^([0-9]{10})$/\1/p" )
if [ -z "${CDATE_OR_NULL}" ]; then
  print_err_msg_exit "\
CDATE_LAST_CYCL must be a string consisting of exactly 10 digits of the 
form \"YYYYMMDD\", where YYYY is the 4-digit year, MM is the 2-digit 
month, DD is the 2-digit day-of-month, and HH is the 2-digit hour-of-
day.
  CDATE_LAST_CYCL = \"$CDATE_LAST_CYCL\""
fi
#
#-----------------------------------------------------------------------
#
...
TEMPLATE_DIR="$PARMDIR/templates"
...
#-----------------------------------------------------------------------
#
# Ensure that the number of fixed files listed in the array FIX_FILES_-
# SYSDIR (which lists the files to be copied from the system directory)
# is equal to the number of fixed files listed in the array FIX_FILES_-
...
#-----------------------------------------------------------------------
#
num_fix_files_sysdir="${#FIX_FILES_SYSDIR[@]}"
num_fix_files_exptdir="${#FIX_FILES_EXPTDIR[@]}"
if [ "${num_fix_files_sysdir}" -ne "${num_fix_files_exptdir}" ]; then
  print_err_msg_exit "\
The number of fixed files specified in FIX_FILES_SYSDIR must be equal to
that specified in FIX_FILES_EXPTDIR:
  num_fix_files_sysdir = ${num_fix_files_sysdir}
  num_fix_files_exptdir = ${num_fix_files_exptdir}
"
else
  NUM_FIX_FILES="${num_fix_files_sysdir}"
fi
#
#-----------------------------------------------------------------------
#
...
##-----------------------------------------------------------------------
##
## Print out list of fixed file names, both on the system 
##
##-----------------------------------------------------------------------
##
#{ cat << EOM >> $SCRIPT_VAR_DEFNS_FP
...
##-----------------------------------------------------------------------
##
## Print the files in the array FIX_FILES_SYSDIR, one per line.
##
##-----------------------------------------------------------------------
##
#{ cat << EOM >> $SCRIPT_VAR_DEFNS_FP
...
##-----------------------------------------------------------------------
##
## Print the files in the array FIX_FILES_EXPTDIR, one per line.
##
##-----------------------------------------------------------------------
##
#{ cat << EOM >> $SCRIPT_VAR_DEFNS_FP
