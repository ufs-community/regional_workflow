...
# input data or HPSS_QUEUE is set (in order to pull data from HPSS at runtime). If 
...
of machine (MACHINE) and external model (EXTRN_MDL_NAME_ICS), and there
is no HPSS_QUEUE specified to pull data from HPSS. Check those values
to ensure that data is available for your machine, or stage your own
data by setting USE_USER_STAGED_EXTRN_FILES = TRUE in config.sh. See
the users guide for additional information on staging input data.
  MACHINE = \"$MACHINE\"
