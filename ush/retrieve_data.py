...
def fill_template(template_str, cycle_date, fcst_hr=0, templates_only=False):
...
def get_requested_files(cla, file_templates, input_loc, method="disk"):
    ...
    target_path = fill_template(cla.output_path, cla.cycle_date)
...
def hpss_requested_files(cla, file_names, store_specs):
    ...
    existing_archive, which_archive = find_archive_files(
        archive_paths,
        archive_file_names,
        cla.cycle_date,
    )
    ...
    for archive_internal_dir_tmpl in archive_internal_dirs:
        archive_internal_dir = fill_template(archive_internal_dir_tmpl, cla.cycle_date)
...
def main(cla):
    ...
    for data_store in data_stores:
        ...
        if data_store == "disk":
            file_templates = (
                cla.file_templates
                if cla.file_templates
                else known_data_info.get("hpss", {}).get("file_names")
            )
...
def parse_args():
    ...
    parser.add_argument(
        "--external_model",
        choices=("FV3GFS", "GSMGFS", "HRRR", "NAM", "RAP", "RAPx", "HRRRx"),
...
if __name__ == "__main__":
    ...
    print(
        f"Running script retrieve_data.py with args:\n", f"{('-' * 80)}\n{('-' * 80)}"
    )
