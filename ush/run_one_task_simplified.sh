...
* From the experiment directory, run the \"make_ics\" task for a specified
  ...
  > cd /path/to/expt/directory
  > ${scrfunc_fn} task_name=\"make_ics\" cdate=\"2019052000\"

* From the experiment directory, run the \"run_post\" task for a specified
  cdate and fhr:

  > cd /path/to/expt/directory
  > ${scrfunc_fn} task_name=\"run_post\" cdate=\"2019052000\" fhr=\"01\"
"
#
#-----------------------------------------------------------------------
#
...
#  qos_default="batch"
  qos_default=$( sacctmgr --noheader show user \
                          user=$USER account=$ACCOUNT format=defaultqos withassoc | \
                 sed -n -r -e "s/^[ ]*([^ ]*)[ ]*$/\1/p" )
...
#-----------------------------------------------------------------------
#
# Set the number of nodes, the MPI processes per node, and the walltime
# for the job to values specified in the workflow variables.
#
#-----------------------------------------------------------------------
#
