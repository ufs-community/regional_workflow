...
then
gridspecs="lambert:306.0:-28.68:-28.68 299.0:835:2500 -34.0:820:2500"
fi

compress_type=c3

if [ $fhr -eq 00 ] ; then
#JAA  ${WGRIB2} BGDAWP${fhr}.${tmmark} | grep -F -f ${PARMfv3}/nam_nests.hiresf_inst.txt | grep ':anl:' | ${WGRIB2} -i -grib inputs.grib${domain}_inst BGDAWP${fhr}.${tmmark}
...
#JAA  ${WGRIB2} BGDAWP${fhr}.${tmmark} | grep -F -f ${PARMfv3}/nam_nests.hiresf_inst.txt | grep 'hour fcst' | ${WGRIB2} -i -grib inputs.grib${domain}_inst BGDAWP${fhr}.${tmmark}
  ${WGRIB2} inputs.grib${domain}_inst -set_bitmap 1 -set_grib_type ${compress_type} \
    -new_grid_winds grid -new_grid_vectors "UGRD:VGRD:USTM:VSTM" \
    -new_grid_interpolation neighbor \
    -new_grid ${gridspecs} ${domain}${RUN}.f${fhr}.${tmmark}.inst
...
#JAA ${WGRIB2} BGDAWP${fhr}.${tmmark} | grep -F -f ${PARMfv3}/nam_nests.hiresf_nn.txt | ${WGRIB2} -i -grib inputs.grib${domain} BGDAWP${fhr}.${tmmark}
${WGRIB2} inputs.grib${domain} -new_grid_vectors "UGRD:VGRD:USTM:VSTM" -submsg_uv inputs.grib${domain}.uv
#JAA ${WGRIB2} BGDAWP${fhr}.${tmmark} -match ":(APCP|WEASD|SNOD):" -grib inputs.grib${domain}.uv_budget
